#!/usr/bin/env Rscript
# fig_shap_stab_vs_a1.R - Generate SD(stability) vs aS figure
# Shows relationship between stability constraint variation and stability parameter
...
# Function to create stability vs a1 plot
plot_shap_stab_vs_a1 <- function(profiles) {
  ...
  # Create the plot
  plot <- ggplot(constraint_summary, aes(x = aS, y = sd_phi_stab)) +
    geom_point(size = CORRELATION_POINT_SIZE, alpha = ALPHA_CORRELATION_POINTS, color = SHAP_COLORS["Stability"]) +
    geom_smooth(method = "gam", se = TRUE, color = SHAP_COLORS["Stability"], 
